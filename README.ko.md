...
- **작업 표시줄 실시간 사용량**: 선택한 서비스의 활성 계정 5시간/7일 사용량을 Windows 작업 표시줄에서 바로 계속 확인합니다 (OpenCode Go는 월간 사용량도 표시).
- **페이스메이커 보기**: 현재 사용 속도가 리셋 시점까지의 이상적인 페이스보다 빠른지 느린지 한눈에 확인해, 속도를 조절하거나 계정을 전환할 타이밍을 잡을 수 있습니다.
...
- **작업 표시줄 사용량**: 5시간/7일 잔여 사용량, 페이스메이커 비교, 수동 새로 고침을 제공하는 상시 표시 사용량 뷰입니다 (OpenCode Go는 월간 사용량도 표시).
...
1. 계정 화면에서 하나 이상의 계정을 추가합니다.
2. 앱이 계정을 검증한 뒤 플랜과 사용량 정보를 확인합니다.
3. 사용할 계정을 선택해 해당 CLI의 활성 계정으로 전환합니다.
4. 실행 중인 세션이 새 인증 파일을 읽도록 안내가 표시되면 해당 도구를 다시 시작합니다.
5. 다른 Windows 설치 환경으로 계정이나 설정을 옮길 때 백업/내보내기 기능을 사용합니다.

계정 전환은 선택한 도구의 로컬 인증 파일을 덮어씁니다. 해당 파일을 직접 관리하거나 다른 도구와 함께 사용 중이라면 먼저 백업해 두세요.
