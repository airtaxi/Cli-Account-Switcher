...
| 프로젝트 | 설명 |
| --- | --- |
| `CodexAccountSwitch.WinUI` | 패키징된 WinUI 3 데스크톱 앱입니다. |
| `CodexAccountSwitch.Api` | Codex OAuth, 인증 문서, 사용량, 모델, API 클라이언트 도우미를 담당합니다. |
| `CodexAccountSwitch.Api.Test` | Codex API 동작을 실험하기 위한 콘솔 프로젝트입니다. |

WinUI 앱은 `net10.0-windows10.0.26100.0`을 대상으로 하며, NativeAOT 게시와 MSIX 도구를 사용하고 `x86`, `x64`, `ARM64` 패키지 번들을 지원합니다.

게시 프로필은 `CodexAccountSwitch.WinUI/Properties/PublishProfiles`에 있습니다.
